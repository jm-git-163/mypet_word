  ...
  const Theme = {
    ...
    apply(level) {
      ...
      /* 풍경은 4:3 그림인데 화면은 세로로 길어, background-size:cover 를 쓰면
         ...
         레벨마다 보이는 가로 위치를 살짝 돌려 늘 똑같지는 않게 하되,
         너무 멀리 돌리면 파라솔 같은 볼거리가 한쪽으로 쏠려 오히려 더
         엉성해 보이므로(예: 22%~78% 전 구간을 쓰면 한쪽 끝에서 파라솔
         하나만 덩그러니 보이고 나머지는 아예 잘려 나감), 가운데에서
         크게 벗어나지 않는 좁은 폭으로만 오갑니다. */
      const px = 42 + ((level * 23) % 17);   // 42%~58% 사이(가운데 근처)만 오갑니다
